	...
	<div class="userList">
		...
		<table class="uList">
			...
			<tr>
				...
				<td class="underline"> 
					<c:if test="${list.user_auth eq'A'.charAt(0) }"> 관리자 </c:if>
					<c:if test="${list.user_auth eq'U'.charAt(0) }"> 일반회원 </c:if>
				</td>
			</tr>
			</c:forEach>
<!-- 			<tr> -->
...
<form id="searchForm" action="/user/user_management.do" method="get">
	...
	<div class="userSearch">
		<select name="type" class="userOpt">
			<option>선택</option>
...
<form id='actionForm' action="/user/user_management.do" method='get'>
	...
	<div class="userPage" >
		<c:if test="${pageMaker.prev }">
			<li class="pfPagination"><a href="<c:out value='${pageMaker.startPage-1}'/>">이전</a></li>
		</c:if>
		<c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
			<li class="pfPagination ${pageMaker.cri.pageNum == num ? "active":"" }" style="float: left; list-style: none; font-weight: bold;"><a href="${num}">${num }</a></li>
		</c:forEach>
		<c:if test="${pageMaker.next }">
			<li class="pfPagination"><a href="<c:out value='${pageMaker.endPage+1}'/>">다음</a></li>
		</c:if>
	</div>
</form>
